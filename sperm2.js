function simulationSketch(p) {
  ...
  let keyToNode = {};
  let assets = [];
  let assetCount = 108; // Set to your number of PNGs
  let spermCells = [];
  let lastShuffle = 0;
  ...
  p.setup = function() {
    ...
    spermCells = [];
    let desiredAssetCount = 666; // set your desired number of sperm cells
    let minSpacing = 10;         // Minimum spacing between pieces
    ...
    for (let i = 0; i < desiredAssetCount; i++) {
      ...
      spermCells.push({
        ...
        scale: p.random(0.333, 0.555),        // Smaller asset scale
  ...
  p.draw = function() {
    p.background(255);
    // p.orbitControl(); // Disable if not needed
  
    if (p.millis() - lastShuffle > shuffleInterval) {
      let swapsPerInterval = 333;
    ...
    for (let y = 0; y < rows_; y++) {
      for (let x = 0; x < cols; x++) {
        ...
        // Define a parameter for maximum pull distance
        let nodePullLimit = 180; // You can adjust this value

        // In your draw loop, when applying node pull:
        for (let node of nodes) {
          let d = p.dist(point.x, point.y, node.x, node.y);
          if (node.force > 0 && d < nodePullLimit) {
            // Use the parameter in your strength calculation:
            let strength = 3 * node.force * (nodePullLimit - d) / nodePullLimit;
    ...
    for (let cell of spermCells) {
      ...
      if (cell.x < -p.width/2) {
        cell.x = -p.width/2;
        cell.vx *= -1;
      } else if (cell.x > p.width/2) {
        cell.x = p.width/2;
        cell.vx *= -1;
      }
      if (cell.y < -p.height/2) {
        cell.y = -p.height/2;
        cell.vy *= -1;
      } else if (cell.y > p.height/2) {
        cell.y = p.height/2;
        cell.vy *= -1;
      }
