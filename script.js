...
function setupGallery() {
    ...
    if (projectContent) {
        ...
        if (images.length > 0) {
            ...
            img.src = adjustPath(images[0].src);
            ...
            prevBtn.addEventListener('click', () => {
                ...
                img.src = adjustPath(images[currentIndex].src);
            });
        ...
        if (page.projects && page.projects.length > 0) {
            page.projects.forEach(project => {
                ...
                link.addEventListener('click', (e) => {
                    e.preventDefault();
                    console.log('Clicking project link:', project.file);
...
async function loadProject(projectFile) {
    ...
    try {
        ...
        // Check if the current page has projects
        if (currentPage.projects && currentPage.projects.length > 0) {
            setupGallery();
        }
    } catch (error) {
